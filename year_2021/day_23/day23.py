...
class State:
    ...
    def enter_hallway(self, source: int) -> Generator[int]:
        yield from (target for target in HALLWAY
    ...
    def enter_room(self, source: int, unit: str) -> int | None:
        ...
        for pos in self.target_rooms[unit]:
            if not self.rooms[pos]:
                ...
                return None
        if not self.path_is_valid(source, ENTRANCES[unit]):
            return None
        return target

    def generate_new_moves(self) -> Generator[State]:
        for source in (s for s in HALLWAY if self.rooms[s]):
        ...
        for room in 'ABCD':
            ...
            if source:
                for target in self.enter_hallway(ENTRANCES[room]):
                    yield self.generate_new_state(source, target)

    def generate_new_state(self, a: int, b: int) -> State:
        _a, _b = minmax(a, b)
        distance = abs(ENTRANCES[self.target_indices[_b]] - _a) + (_b - 7) // 4
        new_cost = self.energy_spent + distance * ENERGY[self.rooms[a]]
